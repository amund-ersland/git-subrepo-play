...
PAUSE "user2 clones and EXCLUDES child-repo-2 via a pathspec, so it is never activated or checked out"
clone_repo $remote_dir/parent-repo $root_dir/user2 --recurse ':(exclude)child-repo-2'

PAUSE "Show the active flags git wrote into user2's LOCAL config (child-repo-2 excluded => inactive)"
...
PAUSE "user2 updates submodules to newest remote WITHOUT --init so the excluded submodule stays inactive"
...
INFO "ℹ️  NOTE: The exclusion lives in the CLONER's local config (via the pathspec), not in .gitmodules — child-repo-2 is still listed in .gitmodules. See s7 for why setting active=false in .gitmodules does NOT achieve this on a recursive clone."
